...
#define THIS_IP            "192.168.133.51"   // the IP users will be connecting to
...
main(int argc, char **argv){
  ...
  while(1){
	...
	std::cout << "Last trigger number: " << triggerNumber-1 << " for Run " << currentRun << "! Processed " << triggerNumber << " triggers!"<< std::endl;
    ...
    if(running){
      ...
      if(eventsStored == 0) continue;
      eventSize = busHandles.mappedMemory[V1742_Event_Size];
      //std::cout << "Ev size: " << eventSize << std::endl;
      std::cout << "Trigger Number #: " << std::hex << "0x" << triggerNumber << " -> dec: " << std::dec << triggerNumber << std::endl;

     
      if(eventSize > 65000){
	std::cout << "Can't handle packets this big...it is likely that the VME is stuck! Stop me and reboot the VME." << std::endl;
	usleep(1000000);
      }
      else{
	unsigned int bufferPos = 0;
	unsigned int tmpEventSize = eventSize;
	while (tmpEventSize > 0) {
	  //std::cout << "Ev size: " << eventSize << std::endl;
	  if(tmpEventSize > 1023){
	    // 	  std::cout << "Ev size 1023: " << eventSize << std::endl;
	    readResult = vme_dma_read(busHandles.busHandle, busHandles.dmaHandle, 0, 0x10000000, VME_A32UD, 0xFFC, 0);
	    memcpy ( &buffer[bufferPos], busHandles.dmaBufferMemory , sizeof(unsigned int)*1023 );
	    tmpEventSize -= 1023;
	    bufferPos += 1023;
	  } 
	  else {
	    for(; bufferPos<eventSize; bufferPos++){
	      buffer[bufferPos] = busHandles.mappedMemory[0];
	      //std::cout << bufferPos << " -> " << std::hex << buffer[bufferPos] << std::dec << std::endl;
	    }
	    tmpEventSize = 0;
	  }
	}
	if(tmpEventSize==0){
	  buffer[2] = triggerNumber;
	  if(writeFile) fwrite(buffer, sizeof(unsigned int), eventSize, fileHandle);
	  sendto(streamingSocket, buffer, sizeof(unsigned int)*eventSize, 0, (struct sockaddr *)&streamingReceiver, sizeof(streamingReceiver));
	  ++triggerNumber;
	}
      }
    }
